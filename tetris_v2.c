 ...
 * tetris.c (v2)
 *
 * A structural refactor of tetris.c (v1). Gameplay and output are unchanged.
 *
 * Structural improvements over tetris.c:
 *   - All mutable game state lives in `struct game_state`, passed by pointer.
 *     No mutable globals remain.
 *   - Uses <stdbool.h> (`bool` / `true` / `false`) instead of TRUE/FALSE macros.
 *   - Input dispatch is extracted into `handle_command`, separating input
 *     parsing from the main game loop.
 *   - Internal functions are marked `static`; the shape table is `const`.
 *   - Related functions are grouped into labelled sections.
 *
 */
