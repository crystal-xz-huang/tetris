 ...
 * tetris_v3.c
 *
 * A graphical refresh of tetris_v2. Gameplay is identical to the original;
 * only the rendering changes.
 *
 * Visual changes from tetris_v2:
 *   - Each cell is now rendered as two "█" block characters, so filled
 *     cells look roughly square (terminal cells are ~2:1 tall:wide).
 *   - The field border uses Unicode box-drawing characters (╔═╗║╚╝).
 *   - The NEXT panel shows a mini 4x2 preview of the upcoming piece
 *     rendered as colored blocks, instead of a single letter.
 *   - The debug view (`?`) is left alone so that it stays scriptable.
 *
 * Rendering in pipe mode: colors still auto-disable when stdout isn't a TTY
 * (or when NO_COLOR is set), but the block glyphs and box-drawing are
 * always emitted — v3's output is intentionally NOT byte-compatible with
 * the original tetris. Use tetris_v2 for the diff-based regression tests.
 */
